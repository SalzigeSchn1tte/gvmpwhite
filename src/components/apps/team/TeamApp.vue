<template>
  <v-ons-page>
    ...
    <section v-show="members.ManagePermission == 1 || members.ManagePermission == 2">
      <v-ons-button class="color-background purple" modifier="large" v-on:click="parkCars">Einparken</v-ons-button>
    </section>

    <section v-show="members.ManagePermission >= 0">
      <v-ons-button class="color-background red" modifier="large" v-on:click="leaveFrak">Fraktion verlassen</v-ons-button>
    </section>
...
<script>
  ...
  export default Apps.register({
    ...
    methods: {
      ...
      parkCars() {
        this.triggerServer("parkFraktionVehicles")
      },
      leaveFrak() {
        this.triggerServer("leaveFrak")
      },
      call(contactdata){
